...
class THMdf : public Std_DF_energy //2
{
 ...
 THMdf(double n0, double *ParmIn, int *OK, int *empty);
 THMdf(double n0, double *ParmIn, int *OK, int *empty, double Emax);
};
...
THMdf :: THMdf(double n0, double *ParmIn, int *OK, int *empty)
{
...
THMdf :: THMdf(double n0, double *ParmIn, int *OK, int *empty, double Emax)
{
 double T0=fabs(ParmIn[2]);
...
class PLWdf : public Std_DF_energy //3
{
 ...
 void FE(double E, double *f, double *f_E);
 void Fp(double p, double *f, double *f_p);
 PLWdf(double *ParmIn, int *OK, int *empty);
};
...
PLWdf :: PLWdf(double *ParmIn, int *OK, int *empty)
{
 double E1=ParmIn[6]*eV*1e6;
 double E2=ParmIn[7]*eV*1e6;
 delta=ParmIn[9];
 nb=ParmIn[12];
...
class DPLdf : public Std_DF_energy //4
{
 ...
 void FE(double E, double *f, double *f_E);
 void Fp(double p, double *f, double *f_p);
 DPLdf(double *ParmIn, int *OK, int *empty);
};
...
DPLdf :: DPLdf(double *ParmIn, int *OK, int *empty)
{
 double E1=ParmIn[6]*eV*1e6;
 double E2=ParmIn[7]*eV*1e6;
 Ebr=ParmIn[8]*eV*1e6;
...
class KAPdf : public Std_DF_energy //6
{
 ...
 void FE(double E, double *f, double *f_E);
 void Fp(double p, double *f, double *f_p);
 KAPdf(double n0, double *ParmIn, int *OK, int *empty);
};
...
KAPdf :: KAPdf(double n0, double *ParmIn, int *OK, int *empty)
{
 double T0=fabs(ParmIn[2]);
 double kappa=ParmIn[4];
 double E_max=ParmIn[7]*eV*1e6;
...
class PLPdf : public Std_DF_energy //7
{
 ...
 void FE(double E, double *f, double *f_E);
 void Fp(double p, double *f, double *f_p);
 PLPdf(double *ParmIn, int *OK, int *empty);
};
...
PLPdf :: PLPdf(double *ParmIn, int *OK, int *empty)
{
 double E1=ParmIn[6]*eV*1e6;
 double E2=ParmIn[7]*eV*1e6;
 delta=ParmIn[9];
...
class PLGdf : public Std_DF_energy //8
{
 ...
 void FE(double E, double *f, double *f_E);
 void Fp(double p, double *f, double *f_p);
 PLGdf(double *ParmIn, int *OK, int *empty);
};
...
PLGdf :: PLGdf(double *ParmIn, int *OK, int *empty)
{
 double E1=ParmIn[6]*eV*1e6;
 double E2=ParmIn[7]*eV*1e6;
 delta=ParmIn[9];
...
//-----------------------------------------------------
// TNP (9) - combined function
// TNG (10) - combined function
// TPL (11) - combined function
// TDP (12) - combined function
//-----------------------------------------------------
...
ELCdf :: ELCdf(double *ParmIn, int *OK)
{
 alpha_c=ParmIn[20]*M_PI/180;
 dmu=ParmIn[22];
...
GAUdf :: GAUdf(double *ParmIn, int *OK)
{
 alpha_c=ParmIn[20]*M_PI/180;
 dmu=ParmIn[22];
...
GABdf :: GABdf(double *ParmIn, int *OK)
{
 alpha_c=ParmIn[21]*M_PI/180;
 dmu=ParmIn[22];
...
SGAdf :: SGAdf(double *ParmIn, int *OK)
{
 alpha_c=ParmIn[21]*M_PI/180;
 dmu=ParmIn[22];
 a4=ParmIn[23];
...
Std_DF :: Std_DF(double ne, double *ParmIn, int k, int *OK, int *empty, int *kap_on, int *Done)
{
 #define InSize 29
 ...
 int E_id=(int)ParmIn[17];
 int mu_id=(int)ParmIn[19];

 *kap_on=0;

 switch (E_id)
 {
  case THM: F1=new THMdf(ne, ParmIn, OK, empty);
	        *Done=1;
			break;

  case PLW: F1=new PLWdf(ParmIn, OK, empty);
	        *Done=1;
			break;

  case DPL: F1=new DPLdf(ParmIn, OK, empty);
	        *Done=1;
			break;
  ...
  case TNP:
  case TNG: {
	         double pcr=mc*sqrt((sqr(kB*ParmIn[2]/mc2+1.0)-1.0)/ParmIn[3]);
	         double Gcr=sqrt(1.0+sqr(pcr/mc));
	         double Ecr=mc2*(Gcr-1.0);
	  
	         if (!k) F1=new THMdf(ne, ParmIn, OK, empty, Ecr);
			 else
			 {
	          double E_max=ParmIn[7]*eV*1e6;
			  double G_max=E_max/mc2+1.0;
			  ...
			  if (E_max>Ecr)
			  {
			   Std_DF_energy *thm=new THMdf(ne, ParmIn, OK, empty);
			   ...
			   if (*OK && !(*empty))
			   {
			    ...
			    double ParmLoc[InSize];
				memcpy(ParmLoc, ParmIn, sizeof(double)*InSize);
			    ParmLoc[6]=Ecr/eV/1e6;
                ParmLoc[12]=nb;

				switch (E_id)
				{
				 case TNT: F1=new PLWdf(ParmLoc, OK, empty);
					       break;
                 case TNP: F1=new PLPdf(ParmLoc, OK, empty);
					       break;
                 case TNG: F1=new PLGdf(ParmLoc, OK, empty);
				}
			   }
  ...
  case KAP: F1=new KAPdf(ne, ParmIn, OK, empty);
	        *Done=1;
			*kap_on=1;
			break;

  case PLP: F1=new PLPdf(ParmIn, OK, empty);
	        *Done=1;
			break;

  case PLG: F1=new PLGdf(ParmIn, OK, empty);
	        *Done=1;
			break;
  ...
  case TDP: if (!k)
			{
			 F1=new THMdf(ne, ParmIn, OK, empty);
			 mu_id=ISO;
			}
			else
			{
			 switch (E_id)
			 {
			  case TPL: F1=new PLWdf(ParmIn, OK, empty);
				        break;
			  case TDP: F1=new DPLdf(ParmIn, OK, empty);
			 }
 ...
 if (*OK && !(*empty))
 {
  ...
  switch (mu_id)
  {
   ...
   case ELC: F2=new ELCdf(ParmIn, OK);
	         break;

   case GAU: F2=new GAUdf(ParmIn, OK);
	         break;

   case GAB: F2=new GABdf(ParmIn, OK);
	         break;

   case SGA: F2=new SGAdf(ParmIn, OK);
	         break;
