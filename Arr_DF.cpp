...
void Arr_DF :: Fp(double p, double p_z, double p_n, double *f, double *df_dp, double *df_dalpha)
{
  ...
  *f=fE/(p*m*G); 
  *df_dp=(dfE_dE-fE*G*m/sqr(p)*(1.0+sqr(p/G/mc)))/sqr(m*G);
  *df_dalpha=0.0;
 }
 else
 {
...
Arr_DF :: Arr_DF(double *ParmIn, int InSize, 
	             int N_E, int N_mu, double *E, double *mu, IDL_DFarr *f3D, 
	             int *OK, int *empty)
{
 ...
 *OK=1;
 ...
 if (N_E<3 || N_mu<3) *empty=1;
 else
 {
  NE=N_E;
  Nmu=N_mu;

  log_on=(int(D1(ParmIn, InSize, 33))==0);
  iso_on=(int(D1(ParmIn, InSize, 32))<2);
  PK_on=(int(D1(ParmIn, InSize, 32))==0);
  LQ_on=int(D1(ParmIn, InSize, 36));
